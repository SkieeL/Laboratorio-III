...
function armaTabla(perfil : string) {
    ...
    if (perfil == "administrador") {
        ...
        for (let i : number = 0; i < usuarios.length; i++) {
            texto += `<tr><td>${usuarios[i].correo}</td><td>${usuarios[i].nombre}</td><td>${usuarios[i].apellido}</td><td>${usuarios[i].perfil}</td><td>${usuarios[i].legajo}</td><td><img src='fotos/${usuarios[i].foto}' height='50px' width='50px'</td><td><input type='button' class='btn btn-danger' value='Eliminar' onclick='borrarUsuario(${usuarios[i].legajo}, "${perfil}")'></td></tr>`;
        }
    }
    else if (perfil == "super_usuario") {
        // Agrega los encabezados
        ...
        // Recorre el array de usuarios agregando los datos de los mismos en la tabla
        for (let i : number = 0; i < usuarios.length; i++) {
            texto += `<tr><td>${usuarios[i].correo}</td><td>${usuarios[i].nombre}</td><td>${usuarios[i].apellido}</td><td>${usuarios[i].perfil}</td><td>${usuarios[i].legajo}</td><td><img src='fotos/${usuarios[i].foto}' height='50px' width='50px'</td><td><input type='button' class='btn btn-danger' value='Eliminar' data-toggle="modal" data-target="#myModalBorrar" onclick='borrarUsuario(${usuarios[i].legajo}, "${perfil}")'></td><td><input type='button' class='btn btn-warning' value='Modificar' data-toggle="modal" data-target="#myModal" onclick='modificarUsuario(${usuarios[i].legajo}, "${perfil}")'></td></tr>`;
        }
    }
    else {
        if (perfil == "invitado") {
    ...
    .done(function (objJson) {
        ...
        for (let i : number = 0; i < usuarios.length; i++) {
            ...
            if (usuarios[i].legajo == legajo) {
                ...
                (<HTMLImageElement>document.getElementById('imgFoto')).src = "img/" + usuarios[i].foto;
